...
function _indexOfFirstOccurrenceInStringWithSlice(haystack: string, needle: string): number {
  if (needle === haystack) return 0;
  const possibilities: { index: number; charsMatching: number }[] = [];

  const haystackArray = [...haystack];
  const needleArray = [...needle];

  let tentativeIndex: number | null = null;
...
// KMP algo
function indexOfFirstOccurrenceInString(haystack: string, needle: string): number {
  if (needle === "") return 0;
...
void _indexOfFirstOccurrenceInStringWithSlice;
export default indexOfFirstOccurrenceInString;
